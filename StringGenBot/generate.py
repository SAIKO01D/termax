...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += "بوت"
    await msg.reply(f"**» استخـراج الجلسـة **{ty}** بواسطـة 𓏺 َِ𝘿َِ𝙇َِ𝙏َِ𝙃َِ𝙊َِ𝙉 . جـارٍ..**.")
    user_id = msg.chat.id
    ...
    text = f"**هذا هو {ty} كـود جلسـة** \n\n`{string_session}` \n\n**مستخـرج مـن :** @DLTHONStringSessionBot\n🍒 **ملاحظـة :** لا تشارك الكود لأحـد لأنـه يستطيع اختراق حسابك من خلاله ❤️ ولا تنسى الانضمام بقناة السورس @DLTHON"
    try:
        if not is_bot:
